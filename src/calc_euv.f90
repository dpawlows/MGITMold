...
subroutine UpdateSecondaryIonization(SecondaryRate)
  ...
  real, dimension(nalts) :: y1, y2
  real :: x1, x2, x, m(nAlts), tDiff(nSecondaryTimes), k(nAlts)
  Integer :: iMin(1)=0


  tDiff = CurrentTime - SecondaryIonTime(1:nSecondaryTimes)

  where (tDiff .lt. 0) tDiff = 1.e20
  iMin = minloc(tDiff)

  if (CurrentTime .lt. SecondaryIonTime(1) .or. CurrentTime .gt. SecondaryIonTime(nSecondaryTimes)) then
     secondaryRate = 0.0
  else
     y1 = SecondaryIonRate(:,iMin(1))
     y2 = SecondaryIonRate(:,iMin(1)+1)
     x1 = 0
...
!       if (currenttime .gt. secondaryiontime(2)+300) then
!        write(*,*) tdiff
!        write(*,*) secondaryiontime(1:nsecondarytimes)
!        write(*,*) itimearray
!          write(*,*) secondaryrate(1),secondaryionrate(1,imin(1)), secondaryionrate(1,imin(1)+1)
!           stop
!        endif
