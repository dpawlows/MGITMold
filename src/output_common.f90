...
cTime = cTimeSave
  ...
  case ('1DALL')
     ...
     nGCs = 0
     nvars_to_write = nSpeciesTotal+nSpecies+nIons+nSpecies+5+nIons-2+1+1+1+1+1
...
contains
  ...
  subroutine output_header
    ...
    if (cType(3:5) == "ALL" .or. cType(3:5) == "ION") then
      ...
      enddo   
      iOff = iOff + nIons-1
      write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "Secondary Ionization Rate"

      write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "Solar Zenith Angle"
      iOff = iOff + 2
...
subroutine output_3dall(iBlock)
  ...
  use ModGITM
  use ModInputs
  use ModEUV, only: EUVIonRates,SZA,SIR
...
!     enddo
        ...
        iOff = iOff + nIons-1
        vars(iOff+1) = sum(SIR(1,1,iialt,:,1))
        Vars(iOff+2) = SZA(1,1,iBlock)
